		...
		if ($no_forms>1) {
			...
			for($i=1;$i<$no_forms;$i++) {
				...
				if ( $this->checkProfile($parameters['pf']) ) {
					...
					$urlAiContactSafe = JURI::base().'index.php?option=com_aicontactsafe&sTask=message&task=message&pf='.$parameters['pf'].'&r_id='.$r_id.'&next_use_ajax=1&format=raw&lang='.$lang;
...
if(substr($joomla,0,3) >= '1.6') {
									$script .= "
												var xCaptcha = new Request({
													...
													method: 'get', 
													onRequest: function(){
																			$('aiContactSafe_form_".$parameters['pf']."').innerHTML = '".JText::_('COM_AICONTACTSAFE_PLEASE_WAIT')."';
													},
									...
									$script .= "
												new Ajax(url, {
													method: 'get',
													onRequest: function(){ 
																			$('aiContactSafe_form_".$parameters['pf']."').setHTML('".JText::_('COM_AICONTACTSAFE_PLEASE_WAIT')."');
																		},
													onComplete: function() { 
