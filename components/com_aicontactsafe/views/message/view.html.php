...
class AiContactSafeViewMessage extends AiContactSafeViewDefault {
	...
	function setVariables() {
		...
		if ($pf == 0) {
			...
			if ( $this->return_to == '' ) {
				$this->return_to = JFilterInput::clean(str_replace('&#38;','&',JRequest::getVar('return_to', '', 'request', 'string')), 'path');
			}
		...
		if ($this->profile->use_captcha == 1 || ($this->profile->use_captcha == 2 && $this->_user_id == 0)) {
			switch($this->profile->captcha_type) {
				case 0:
					...
					$change_image = JText::_('COM_AICONTACTSAFE_NOT_READABLE?_CHANGE_TEXT');
					?>
					<div id="div_captcha">
